################################################################################
# PELSA UniProt fetch + feature-cache + per-species refresh.
#
# All tests here target R/tab_pelsa_uniprot_helpers.R. The species
# classification/resolution suite (R/tab_pelsa_species_resolve.R) was split out
# to test-pelsa-species.R.
#
# Consolidated OFFLINE suite. Merged verbatim from three former files:
...
#
# Every suite is hermetic: injected fetch_fn / mocked httr2. NO real network is
# touched. The benchmark "network" is a calibrated arithmetic model, never a
# real request.
#
...
# no name collisions across the files (verified at merge time), so no helper was
# renamed and no source()/`%||%` line needed de-duplication. Only a single
# `library(testthat)` is kept (was duplicated across two of the files).
################################################################################
...
test_that("the failed-condition block keeps the req_error(>=500) policy + NA/5xx guard", {
  src_path <- testthat::test_path("..", "..", "R", "tab_pelsa_uniprot_helpers.R")
  skip_if_not(file.exists(src_path), "tab_pelsa_uniprot_helpers.R source not found")
