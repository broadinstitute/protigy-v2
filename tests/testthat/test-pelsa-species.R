################################################################################
# PELSA species classification + resolution (taxonomy-code convention).
#
# Split out of test-pelsa-uniprot-cache.R: these tests target
# R/tab_pelsa_species_resolve.R, a separate source file from the UniProt
# fetch/refresh helpers (R/tab_pelsa_uniprot_helpers.R).
#
# Hermetic: every test injects a `validate_fn` stub; the taxonomy API is NEVER
# hit live. A folder named by digits is a UniProt taxon code; a named folder is
# self-curated.
################################################################################
