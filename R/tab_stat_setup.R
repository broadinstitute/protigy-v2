...
statSetup_Tab_Server <- function(id = "statSetupTab",GCTs_and_params, globals,GCTs_original){
  ...
  moduleServer(id, function (input, output, session) {
    already_loaded <- reactiveVal(NULL)
    ## GATHERING INPUTS ##
    ...
    # 
    #   current[[ome]]$contrasts <- selected_contrasts_reactive()
    #   current[[ome]]$contrasts_box <- input$contrast_input_id
    #   current[[ome]]$flips_box <- input$flip_id
    #   stat_param(current)
    # })
    ...
    selected_contrasts_reactive <- reactive({
      req(input$stat_setup_annotation, selected_ome())
      ...
      # # Save into ome-specific slot
      # current <- stat_param()  # get current value
      # current[[selected_ome]]$contrasts <- selected_contrasts
      # stat_param(current)  # write it back
    })
    

    # observe({
    #   req(selected_ome(),input$contrast_input_id)
    #   pairwise_contrasts <- combn(input$stat_setup_annotation, 2, simplify = FALSE)
    #   saved_contrasts <- stat_param()[[selected_ome()]]$contrasts_box
    #   if (is.null(saved_contrasts)) {saved_contrasts <- pairwise_contrasts}
    #   updateCheckboxGroupInput(session, "contrast_input_id", selected = saved_contrasts)
    # })
    # 
    # observe({
    #   req(selected_ome(),input$flip_id)
    #   saved_flips <- stat_param()[[selected_ome()]]$flips_box
    #   if (is.null(saved_flips)) {saved_flips <- NULL}
    #   updateCheckboxGroupInput(session, "flip_id", selected = saved_flips)
    # })
