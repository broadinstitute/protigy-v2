################################################################################
# Module: PELSA - Section 3
#
# Third section of the PELSA tab. Displays one inner tab per ome (per-ome
...
PELSASection3_Ome_Server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    ## MODULE SERVER LOGIC ##
    # TODO: build out Section 3 analysis for a single ome here.

    output$section_contents <- renderUI({
      pelsa_placeholder_box(
        ns      = ns,
        ome     = ome,
        title   = "PELSA - Section 3",
        message = "This section will house the third part of the PELSA workflow."
      )
    })
