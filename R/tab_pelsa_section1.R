################################################################################
# Module: PELSA - Section 1
#
# First section of the PELSA tab. Displays one inner tab per ome (per-ome
...
# server returns no exports. Build out the analysis by:
#   1. Adding controls/plots/tables to PELSASection1_Ome_UI / _Ome_Server.
#   2. Adding pure helpers to tab_pelsa_helpers.R (or a section-specific helper).
#   3. Returning a named list of export functions from _Ome_Server.
#   4. Wiring new imports into R/protigy-package.R + `devtools::document()`.
#
# Section/sub-tab title shown in the navbar: see app_ui.R PELSA navbarMenu.
################################################################################
    ...
    output$ome_tabset_box <- renderUI({
      ...
      add_css_attributes(
        shinydashboardPlus::box(
          PELSASection1_Ome_UI(id = ns(ome), ome = ome),
          width = 12
        ),
...
PELSASection1_Ome_Server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    ## MODULE SERVER LOGIC ##
    # TODO: build out Section 1 analysis for a single ome here.

    output$section_contents <- renderUI({
      pelsa_placeholder_box(
        ns      = ns,
        ome     = ome,
        title   = "PELSA - Section 1",
        message = "This section will house the first part of the PELSA workflow."
      )
    })
