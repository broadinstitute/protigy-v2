...
set_annot_colors <- function( annot_table,
  ...
  if (autodetect_continuous) {
    ...
    is.continuous <- function ( annot_col, na_annot_vals, nfactor_cutoff=10 ) {
      annot_vals = sort(unique(annot_col)) # find unique annot_vals
...
set_annot_colors_discrete <- function( annot_table, # ====
  ...
  for ( annot in annot_names  ) {
    annot_vals <- sort(unique( annot_table[[annot]] )) # sort unique annotations (sorting prevents different color assignments based on order-of-appearanace)
    ...
    if( length( annot_vals ) == 2 || # if we have a pair, 
      ...
      if ( n_colors > length(unlist(qual.pals)) ) { # if we have too many annotations
        annot_colors = rep(na_color, n_colors) # PLACEHOLDER-- just make a vector of blank colors
        warning(paste0("For the annotation ", annot, ", there are ", n_colors," unique annotation values-- but this function can only assign ",
                       length(unlist(qual.pals)), " unique colors.",
                       " All values have been assigned the 'NA' color, '", na_color, "'). This is a placeholder behavior."))
        
      } else if (sum(!grepl("^[0-9]+$", annot_vals), na.rm=TRUE) <= length(na_index)) { # if we have SEQUENTIAL data (i.e. its JUST digits)
        ...
        while ( n_colors > length(qual.colors) ) { # if there aren't enough colors in this palette
          qual.index = (qual.index+1) %% length(qual.pals) # increase qual.index
          qual.count=qual.index+1 # re-index qual.count
          qual.colors = c(qual.colors, qual.pals[[qual.count]]) # keep tacking on new palates until we have enough colors
        }
