...
statSummary_Tab_Server <- function(id = "statSummaryTab",
  ...
  moduleServer(id, function (input, output, session) {
    ...
    output$ome_tabset_box <- renderUI({
      req(shared$chosen_omes)
      
      # generate a tab for each -ome
      tabs <- lapply(shared$chosen_omes, function(ome){
        tabPanel(
          title = ome,
    ...
    observe({
      updateTabsetPanel(inputId = "ome_tabs")
    })
    
    # call the server function for each individual ome
    all_plots <- reactiveVal() # initialize
    observeEvent(shared$chosen_omes, {
      output_plots <- sapply(shared$chosen_omes, function(ome) {
